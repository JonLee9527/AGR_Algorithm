#ifndef DJI_ALGRICULTURE_SHAPE_H_H
#define DJI_ALGRICULTURE_SHAPE_H_H
...
public:
	...
	POINT(DjiPoint pt){ x = pt.x; y=pt.y; }
	POINT& operator = (const DjiPoint &pt){ 
...
public:
	...
	//s(x1,y1) e(x2,y2)     (y2-y1)x+(x1-x2)y+x2y1-x1y2=0;
	LINE(DjiLineSeq seq)
	{
		a = seq.e.y - seq.s.y;
		b = seq.s.x - seq.e.x;
		c=seq.e.x*seq.s.y-seq.s.x*seq.e.y;
	}

	LINE(DjiLine line)
	{
		*this=LINE(DjiLineSeq(line.pt, line.pt + line.vec));
	}
};
...
{
	return((crossmultiply(l.e, p, l.s) == 0) && (((p.x - l.s.x)*(p.x - l.e.x) <= 0) && ((p.y - l.s.y)*(p.y - l.e.y) <= 0)));
}
...
{
	return abs(crossmultiply(p, l.e, l.s)) / dist(l.s, l.e);
}
/* 计算点到折线集的最近距离,并返回最近点.
...
{
	return crossmultiply(u.s, v.e, v.s)*crossmultiply(v.e, u.e, v.s) >= 0;
}
...
void checkconvex(int vcount, POINT polygon[], bool bc[])
{
	...
	while (count) // 判断凸凹性 
	{
		if (crossmultiply(polygon[(index + 1) % vcount], polygon[(index + 2) % vcount], polygon[index]) >= 0)
			bc[(index + 1) % vcount] = 1;
		else
			bc[(index + 1) % vcount] = 0;
		index++;
...
int insidepolygon(int vcount, POINT Polygon[], POINT q)
{
	...
	for (i = 0; i<vcount; i++)
	{
		...
		if ((bintersect_a = intersect_A(l1, l2)) || // 相交且不在端点 
			...
			((r1 = crossmultiply(Polygon[i], Polygon[(i + 1) % n], l1.s)*crossmultiply(Polygon[(i + 1) % n], Polygon[(i + 2) % n], l1.s))>0) ||
			(bonline3 = online(l1, Polygon[(i + 2) % n])) &&     /* 下一条边是水平线，前一个端点和后一个端点在射线两侧  */
			((r2 = crossmultiply(Polygon[i], Polygon[(i + 2) % n], l1.s)*crossmultiply(Polygon[(i + 2) % n],
...
bool InsideConvexPolygon(int vcount, POINT polygon[], POINT q) // 可用于三角形！ 
{
	...
	for (i = 0; i<vcount; i++)
	{
		...
		if (crossmultiply(p, l.e, l.s)*crossmultiply(q, l.e, l.s)<0) /* 点p和点q在边l的两侧，说明点q肯定在多边形外 */
			break;
	}
	return (i == vcount);
}
...
void Graham_scan(POINT PointSet[], POINT ch[], int n, int &len)
{
	...
	for (i = 1; i<n - 1; i++) /* 对顶点按照相对PointSet[0]的极角从小到大进行排序，极角相同的按照距离PointSet[0]从近到远进行排序 */
	{
		k = i;
		for (j = i + 1; j<n; j++)
		if (crossmultiply(PointSet[j], PointSet[k], PointSet[0])>0 ||  // 极角更小    
			(crossmultiply(PointSet[j], PointSet[k], PointSet[0]) == 0) && /* 极角相等，距离更短 */
	...
	for (i = 3; i<n; i++)
	{
		while (crossmultiply(PointSet[i], ch[top], ch[top - 1]) >= 0)
			top--;
		ch[++top] = PointSet[i];
	}
	len = top + 1;
...
POINT gravitycenter(int vcount, POINT polygon[])
{
	...
	for (int i = 1; i<vcount - 1; i++)
	{
		...
		cs = crossmultiply(polygon[i], polygon[i + 1], polygon[0]) / 2;
		//三角形面积可以直接利用该公式求解 
		if (abs(s)<1e-20)
		{
...
void pointtangentpoly(int vcount, POINT polygon[], POINT p, POINT &rp, POINT &lp)
{
	...
	for (int i = 1; i<vcount; i++)
	{
		...
		blp = crossmultiply(ep.e, p, ep.s) >= 0;                // p is to the left of pre edge 
		bln = crossmultiply(en.e, p, en.s) >= 0;                // p is to the left of next edge 
		if (!blp&&bln)
		{
			if (crossmultiply(polygon[i], rp, p)>0)           // polygon[i] is above rp 
				rp = polygon[i];
		}
		if (blp&&!bln)
		{
			if (crossmultiply(lp, polygon[i], p)>0)           // polygon[i] is below lp 
				lp = polygon[i];
		}
	}
	return;
